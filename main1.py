...
#         dense_score = cosine_similarity(user_vector, emb_vec)
#         similarities.append((dense_score, chunk_text))
...
#     user_query_vec = vectorizer.transform([prompt])
#     sparse_scores = cosine_similarity(tfidf_matrix, user_query_vec).flatten()
...
#         score = dense_score
#     else:
#         score = (sparse_scores + dense_score) / 2
...
#     prompt = data.get("prompt", "")
#     filename = data.get("filename", "")
#     retrieval_method = data.get("retrieval_method", "dense")  # default to dense
...
#     for chunk_text, emb_str in items:
#         emb_vec = np.fromstring(emb_str.strip("[]"), sep=",")
#         score = cosine_similarity_dense(user_vector, emb_vec)
...
#         print("Using sparse retrieval", final_scores)
#     elif retrieval_method == "dense":
#         final_scores = dense_scores
#         print("Using dense retrieval", final_scores)
#     else:  # hybrid
#         final_scores = (dense_scores + sparse_scores) / 2
#         print("Using hybrid retrieval", final_scores)

#     # --- Rank and pick top chunks ---
#     ranked_indices = np.argsort(-final_scores)
#     top_chunks = [corpus[i] for i in ranked_indices[:3]]
#     rag_context = "\n\n".join(top_chunks)

#     # --- Add file-specific instruction if provided ---
#     file_instruction = ""
#     if filename:
#         item = db.query(FileData).filter(FileData.name == filename).first()
#         if item:
...
# #     prompt = data.get("prompt", "")
# #     filename = data.get("filename", "")
# #     retrieval_method = data.get("retrieval_method", "dense")  # default dense
...
# #     for chunk_text, emb_str in items:
# #         emb_vec = np.fromstring(emb_str.strip("[]"), sep=",")
# #         dot = np.dot(user_vector, emb_vec)
# #         score = dot / (norm(user_vector) * norm(emb_vec) + 1e-9)
# #         dense_scores.append(score)
# #     dense_scores = np.array(dense_scores)
...
# #     # --- Your chatbot code continues here ---
